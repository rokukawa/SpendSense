...
export const postConta = async (req, res) => {    
    ...
    const result = schema.validate({data_criacao:data_criacao, saldo:saldo, nome_conta: nome_conta})
...
export const putConta = async (req, res) => {    
    ...
    const result = schema.validate({saldo:saldo, nome_conta: nome_conta})
    if (result.error){
        res.json({status: false, mensagem: result.error.message})
    }else{
...
export const postCriarConta = async (req, res) => {
    ...
    try {
        ...
        if(response.data.status){
            ...
            res.render('criar-conta', {mensagem: response.data.mensagem});
        }
    } catch (error) {
...
export const postListarConta = async (req, res) => {
    ...
    if(editar !== undefined){
        ...
        if(response.data.status){
            ...
            res.render('listar-conta', {data: conta.data.conta, mensagem: response.data.mensagem});
        }
    }else{
